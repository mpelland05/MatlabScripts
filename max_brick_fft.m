...
for ss = 1:length(nsubj),
	temp = load(strcat(bpath,fs,nsubj{ss})); temp = temp.ets;%temp = temp.tseries;
...
for se = 1:length(seeds),
    ...
    tt = fft_results{se};
    ffts{se}= [mean(tt(:,6:35),2) mean(tt(:,38:67),2) mean(tt(:,70:98),2) mean(tt(:,102:130),2) mean(tt(:,133:162),2) mean(tt(:,165:194),2)];
    xlswrite(strcat(bpath,fs,'FFT',fs,'FFT_output_binned.xls'),ffts{se}, strcat('s',num2str(seeds(se))) );
